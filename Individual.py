...
def writeToResults(name, s, c):
	...
	results = list()
	with open("Individual_{}_{}.txt".format(name, c), "r") as f:
		results = f.readlines()
	
	results.append(s)
	
	with open("Individual_{}_{}.txt".format(name, c), "w") as f:
		for i in results:
			f.write(i)
...
class Data:
	...
	def prepare(self, topic, c):
		...
		f = open(cur_dir + "PickledData" + "/{}_{}.pkl".format(topic, c), 'wb')
		pickle.dump(self, f)
		f.close()
...
class Classifier:
	...
	def classify(self, dt, topic, ev, sw, c):
		...
		f = open(cur_dir + "PickledData" + "/{}_{}_{}.pkl".format(self.name, topic, c), 'wb')
		pickle.dump(self, f)
		f.close()
...
if __name__ == "__main__":
	...
	for c in range (0, 5):
		for n, j in enumerate(classifiers):
			...
			with open("Individual_{}_{}.txt".format(names[n], c), "w") as f:
			...
			for i in topics:
				...
				if "{}_{}.pkl".format(i, c) in os.listdir(cur_dir + "PickledData"):
					f = open(cur_dir + "PickledData" + "/{}_{}.pkl".format(i, c), 'rb')
					dt = pickle.load(f)
					f.close()
				else:
					dt.prepare(i, c)
	...
	for i in names:
		with open("cross_validated_{}".format(i), "w") as f:
			f.write("{0:10} : {1:10} | {2:10} | {3:10} | {4:10} | {5:10} | \n".format("Topic", "Recall", "Precision", "F5", "SO", "Time"))
	...
	for i in names:
		...
		for j in range(0,5):
			with open("Individual_{}_{}.txt".format(i, j), "r") as f:
				r = f.readlines()
		...
		for t in topics:
			...
			with open("cross_validated_{}".format(i), "r") as f:
				results = f.readlines()
