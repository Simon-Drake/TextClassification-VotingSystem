...
def writeToResults(name, s, c):
	...
	results = list()
	with open("UnderSample_{}_{}.txt".format(name, c), "r") as f:
		results = f.readlines()
	
	results.append(s)
	
	with open("UnderSample_{}_{}.txt".format(name, c), "w") as f:
		for i in results:
			f.write(i)
...
class Data:
	...
	def prepare(self, topic, c):
		...
		# taking 50% of the data at random
		ntrns = math.floor(len(rtrvdStds) * 0.5)
		...
		zero_count = 0
		one_count = 0
		for i in self.train_classes:
			if i == 0:
				zero_count += 1
			else:
				one_count += 1

		# This section undersampled the data
		traincopy = self.train_classes.copy()	
		zeros = list()
		# If the class is imbalanced:
		if zero_count > one_count:
			# Log the negative instances
			for i, item in enumerate(traincopy):
				if item == 0:
					zeros.append(i)
			...
			remove = list()
			# Number to remove 
			to_remove = zero_count - one_count
			for i in range(0, to_remove):
				# If we have not removed enough remove another on at random
				if to_remove >0:
			...
			self.train_classes = [j for i,j in enumerate(self.train_classes) if i not in remove]
			train_corpus = [j for i,j in enumerate(train_corpus) if i not in remove]
		...
		f = open(cur_dir + "PickledData" + "/Undersample_{}_{}.pkl".format(topic, c), 'wb')
		pickle.dump(self, f)
		f.close()
...
class Classifier:
	...
	def classify(self, dt, topic, ev, sw, c):
		...
		f = open(cur_dir + "PickledData" + "/Undersample_{}_{}_{}.pkl".format(self.name, topic, c), 'wb')
		pickle.dump(self, f)
		f.close()
...
if __name__ == "__main__":
	...
	classifiers = [
		...
		AdaBoostClassifier(n_estimators=400, algorithm='SAMME.R'),

		MLPClassifier(hidden_layer_sizes=(150,), max_iter=600, solver='lbfgs'),
	...
	for c in range (0, 5):
		for n, j in enumerate(classifiers):
			...
			with open("UnderSample_{}_{}.txt".format(names[n], c), "w") as f:
			...
			for i in topics:
				...
				if "Undersample_{}_{}.pkl".format(i, c) in os.listdir(cur_dir + "PickledData"):
					f = open(cur_dir + "PickledData" + "/Undersample_{}_{}.pkl".format(i, c), 'rb')
					dt = pickle.load(f)
					f.close()
				else:
					dt.prepare(i, c)
	...
	for i in names:
		with open("cross_validated_undersample_{}".format(i), "w") as f:
			f.write("{0:10} : {1:10} | {2:10} | {3:10} | {4:10} | {5:10} | \n".format("Topic", "Recall", "Precision", "F5", "SO", "Time"))
	...
	for i in names:
		...
		for j in range(0,5):
			with open("UnderSample_{}_{}.txt".format(i, j), "r") as f:
				r = f.readlines()
		...
		for t in topics:
			...
			with open("cross_validated_undersample_{}".format(i), "r") as f:
				results = f.readlines()
