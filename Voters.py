...
def writeToResults(s, c):
	results = list()
	with open("Voterz_{}.txt".format(c), "r") as f:
		results = f.readlines()
	
	results.append(s)
	
	with open("Voterz_{}.txt".format(c), "w") as f:
		for i in results:
			f.write(i)
...
if __name__ == "__main__":
	...
	# Voters is a nested dictionary {validation run :{classifier : {topic : predictions}}}
	voters = dict()
	test_classes = dict()
	...
	for c in range (0, 5):
		...
		for n, j in enumerate(classifiers):
			...
			for i in topics:
				...
				f = open(cur_dir + "PickledData" + "/{}_{}_{}.pkl".format(names[n], i, c), 'rb')
				cl = pickle.load(f)
				f.close()
	...
	for i in range(0, 5):
		...
		with open("Voterz_{}.txt".format(i), "w") as f:
	...
	def compute_stats(y,z):
		...
		# Retrieve data
		for j in range(0,5):
			with open("Voterz_{}.txt".format(j), "r") as f:
				r = f.readlines()
			for n in r[y:z]:
				x = n.split(":")
				topic = x[0].strip()
